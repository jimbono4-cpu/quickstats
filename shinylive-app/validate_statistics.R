...
# ============================================================================
# TEST 13: DAG / dagitty adjustment sets
# ============================================================================
cat("--- TEST 13: DAG Adjustment Sets ---\n")

if (!requireNamespace("ggdag", quietly = TRUE)) {
  cat("  [SKIP] ggdag not installed in this environment (available in WebR only)\n")
  cat("  Testing dagitty directly instead...\n\n")

  if (requireNamespace("dagitty", quietly = TRUE)) {
    library(dagitty)

    # Build a simple DAG: X -> Y, C -> X, C -> Y (C is a confounder)
    dag_str <- dagitty('dag { X -> Y; C -> X; C -> Y }')
    dagitty::exposures(dag_str) <- "X"
    dagitty::outcomes(dag_str) <- "Y"

    record("dagitty DAG creation succeeds", !is.null(dag_str))

    adj_sets <- dagitty::adjustmentSets(dag_str)
    record("adjustmentSets computes successfully",
           length(adj_sets) > 0)

    adj_vars <- unlist(lapply(adj_sets, as.character))
    record("Adjustment set includes confounder C",
           "C" %in% adj_vars,
           sprintf("sets: %s", paste(sapply(adj_sets, paste, collapse=","), collapse="; ")))

    # No confounding: X -> Y only
    dag_noconf <- dagitty('dag { X -> Y }')
    dagitty::exposures(dag_noconf) <- "X"
    dagitty::outcomes(dag_noconf) <- "Y"
    adj_noconf <- dagitty::adjustmentSets(dag_noconf)
    record("No-confounder DAG returns empty adjustment set",
           length(adj_noconf) == 0 || (length(adj_noconf) == 1 && length(adj_noconf[[1]]) == 0))

    # Collider: X -> M, Y -> M (should NOT adjust for M)
    dag_collider <- dagitty('dag { X -> Y; X -> M; Y -> M }')
    dagitty::exposures(dag_collider) <- "X"
    dagitty::outcomes(dag_collider) <- "Y"
    adj_collider <- dagitty::adjustmentSets(dag_collider)
    collider_vars <- unlist(lapply(adj_collider, as.character))
    record("Collider DAG: M not in adjustment set",
           is.null(collider_vars) || !("M" %in% collider_vars),
           sprintf("sets: %s", if (length(adj_collider) == 0) "empty (correct)"
                   else paste(sapply(adj_collider, paste, collapse=","), collapse="; ")))
  } else {
    cat("  [SKIP] dagitty also not installed. DAG tests skipped entirely.\n")
  }
} else {
  library(ggdag)
  ...
  record("dagify creates valid DAG object",
         !is.null(dag))

  record("DAG has underlying dagitty object",
         !is.null(dag$dag))

  adj_sets <- dagitty::adjustmentSets(dag$dag)
  record("adjustmentSets computes successfully",
         length(adj_sets) > 0)
  ...
  dag_noconf <- ggdag::dagify(Y ~ X, exposure = "X", outcome = "Y")
  adj_noconf <- dagitty::adjustmentSets(dag_noconf$dag)
  record("No-confounder DAG returns empty adjustment set",
         length(adj_noconf) == 0 || (length(adj_noconf) == 1 && length(adj_noconf[[1]]) == 0))

  dag_collider <- ggdag::dagify(
    Y ~ X,
    M ~ X + Y,
    exposure = "X",
    outcome = "Y"
  )
  adj_collider <- dagitty::adjustmentSets(dag_collider$dag)
  collider_vars <- unlist(lapply(adj_collider, as.character))
  record("Collider DAG: M not in adjustment set",
         !("M" %in% collider_vars),
         sprintf("sets: %s", if (length(adj_collider) == 0) "empty (correct)"
