    ...
    observeEvent(input$fit_model, {
      ...
      tryCatch({
        mod <- switch(input$model_type,
          ...
          "glm" = {
            # For logistic: ensure outcome is 0/1
...
causal_server <- function(id, shared) {
  moduleServer(id, function(input, output, session) {
    ...
    # Compute adjustment sets via dagitty backend
    output$adjustment_info <- renderUI({
      ...
      tryCatch({
        # Use dagitty::adjustmentSets on the underlying dagitty object
        dagitty_obj <- dag$dag
        adj <- dagitty::adjustmentSets(dagitty_obj)
...
results_server <- function(id, shared) {
  moduleServer(id, function(input, output, session) {
    ...
    generate_report <- function() {
      ...
      if (!is.null(shared$model_result)) {
        ...
        for (i in seq_len(nrow(tidy_df))) {
          row <- tidy_df[i, ]
          pval <- if (row$p.value < 0.001) "<0.001" else as.character(round(row$p.value, 4))
          lines <- c(lines, sprintf("| %s | %.4f | %.4f | %.3f | %s |",
            row$term, row$estimate, row$std.error, row$statistic, pval))
        }
        lines <- c(lines, "")
      }
    ...
    # Preview report in textarea
    observeEvent(input$preview_report, {
      md <- generate_report()
      session$sendCustomMessage("updateReportText",
        list(id = ns("report_text"), text = md))
    })

    # Download report
    observeEvent(input$download_report, {
      md <- generate_report()
      session$sendCustomMessage("downloadMarkdown",
        list(content = md, filename = paste0("analysis_report_", Sys.Date(), ".md")))
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$script(HTML("
      ...
      Shiny.addCustomMessageHandler('downloadMarkdown', function(msg) {
